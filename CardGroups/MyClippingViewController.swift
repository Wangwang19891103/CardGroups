...
class MyClippingViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UISearchBarDelegate{
...
//        let tap: UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(MyClippingViewController.dismissKeyboard))
...
//        UIApplication.sharedApplication().statusBarHidden=true;
        ...
        firebase.child(owner).queryOrderedByChild("clipping").queryEqualToValue(true).observeEventType(.Value, withBlock: { snapshot in
            self.photos = []            
...
//    }
    ...
    @IBAction func clippingClick(sender: AnyObject) {
        ...
        if clipState {
            ...
            self.firebase.child(self.owner).child(click_title).updateChildValues(["clipping": false])
...
//                    self.firebase.child("cards").child(click_title).updateChildValues(["favorite": false])
            
            (sender as! UIButton).setImage(UIImage(named: "Heart Off Icon"), forState: .Normal)
            self.firebase.child(self.owner).child(click_title).updateChildValues(["favorite": false])
...
//                    self.firebase.child("cards").child(click_title).updateChildValues(["favorite": true])
            
            (sender as! UIButton).setImage(UIImage(named: "Heart On Icon"), forState: .Normal)
            self.firebase.child(self.owner).child(click_title).updateChildValues(["favorite": true])
...
extension MyClippingViewController {
...
extension MyClippingViewController : PinterestLayoutDelegate {
